---
title: "Tutoral de uso de phyml3 para selección de modelos y búsquedas intensivas de árboles"
...
phyml -i primates.phy -m 000000 -o n &> /dev/null
...
pchisq(xsq,1)      # 0.9876227;  este es el valor critico de la X2 para el estadistico LRT con 1 grado de libertad; [1] 1
1-(pchisq(xsq,1))  # 0.01237734 esta es la p asociada a dicho valor crítico, SIGNIFICATIVA con alpha = 0.05, pero no con alpha = 0.01
...
# despliega el árbol: figtree primates.phy_phyml_tree.txt &
```
...
phyml -i primates_21_AA.phy -d aa -m BLOSUM62 -c 1 
...
for mat in BLOSUM62 DAYHOFF JTT LG MTMAM WAG; do
     echo " >>> running: phyml -i primates_21_AA.phy -d aa -m $mat -u primates_Blosum62.ph -c 1 -v 0 -o r &> /dev/null"
     phyml -i primates_21_AA.phy -d aa -m $mat -u primates_Blosum62.ph -c 1 -o r &> /dev/null     
     grep Log- *21_AA*stats.txt > ${mat}.fit

     echo " >>> running: phyml -i primates_21_AA.phy -d aa -m $mat -c 4 -a e -u primates_Blosum62.ph -o r &> /dev/null"
     phyml -i primates_21_AA.phy -d aa -m ${mat} -c 4 -a e -u primates_Blosum62.ph -o r &> /dev/null     
     grep Log- *21_AA*stats.txt > ${mat}+G.fit

     echo " >>> running: phyml -i primates_21_AA.phy -d aa -m $mat -f e -c 1 -u primates_Blosum62.ph -o r &> /dev/null"
     phyml -i primates_21_AA.phy -d aa -m $mat -f e -c 1 -u primates_Blosum62.ph -o r &> /dev/null     
     grep Log- *21_AA*stats.txt > ${mat}+f.fit

     echo " >>> running: phyml -i primates_21_AA.phy -d aa -m $mat -u primates_Blosum62.ph -f e -a e -o r &> /dev/null"
     phyml -i primates_21_AA.phy -d aa -m $mat -u primates_Blosum62.ph -f e -a e -c 4 -o r &> /dev/null     
...
```{r chunk2, out.width = '800px', fig.retina = NULL, fig.align='center', echo=TRUE}
...
# finalmente, hagamos una busqueda intensiva bajo este modelo, usando el algoritmo BEST the rearreglo de ramas, con 5 arboles de similla y -o tlr
phyml -i primates_21_AA.phy -d aa -m JTT -c 4 -a e -f e -s BEST --rand_start --n_rand_starts 5
 
# despliega y edita el árbol figtree primates_21_AA.phy_phyml_tree.txt &
```
